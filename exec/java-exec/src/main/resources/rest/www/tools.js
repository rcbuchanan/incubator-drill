...
$(window).load(function () {
	...
	timetable.sort(function (r1, r2) {
	    if (r1.category == r2.category) return r1.name > r2.name;
	...
	renderer.drawNodes(function(graph, root) {
		...
		var color = graph.node(u).label.endsWith("Exchange") ? "white" : d3.rgb(globalconfig.majorcolorscale(graph.node(u).fragment));
	...
	var result = renderer.layout(layout).run(graph, svg.append("g"));
	...
	buildplangraph(d3.select("#graph-canvas"), profile.plan);
	buildtimingchart(d3.select("#timing-canvas"), extractminortimes(profile));
