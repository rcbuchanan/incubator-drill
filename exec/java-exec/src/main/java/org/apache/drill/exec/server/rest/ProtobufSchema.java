 ...
 */
public class ProtobufSchema extends AbstractSchema {
  final Class clazz;
  private Object target;

  /**
   * Creates a ProtobufSchema.
   * 
   * @param target
   *          Object whose fields will be sub-objects of the schema
   */
  public ProtobufSchema(Object target) {
    super();
  ...
  public String toString() {
    return "ProtobufSchema(target=" + target + ")";
  }
  ...
  protected Map<String, Table> getTableMap() {
    ...
    for (Method method : clazz.getMethods()) {
      ...
      final Table table = accessorRelation(method);
      if (table == null) {
        continue;
      }
      builder.put(methodName, table);
    }
  ...
  /**
   * Returns an expression for the object wrapped by this schema (not the schema
   * itself).
   */
  Expression getTargetExpression(SchemaPlus parentSchema, String name) {
    return Types.castIfNecessary(target.getClass(), Expressions.call(Schemas
        .unwrap(getExpression(parentSchema, name), ProtobufSchema.class),
        BuiltinMethod.REFLECTIVE_SCHEMA_GET_TARGET.method));
  }

  /**
   * Returns a table based on a particular field of this schema. If the field is
   * not of the right type to be a relation, returns null.
   */
  ...
  /**
   * Returns a table based on an accessor in this schema. If the field is
   * not of the right type to be a relation, returns null.
   */
  private <T> Table accessorRelation(final Method accessor) {
    final Type elementType = getElementType(accessor.getReturnType());
    if (elementType == null) {
      return null;
    }
    Object o;
    try {
      o = accessor.invoke(target);
    } catch (IllegalArgumentException | ReflectiveOperationException e) {
      throw new RuntimeException("Error while invoking accessor " + accessor, e);
    }
    @SuppressWarnings("unchecked")
    final Enumerable<T> enumerable = toEnumerable(o);
    return new AccessorTable<T>(accessor, elementType, enumerable);
  }
  ...
  private static Enumerable toEnumerable(final Object o) {
    ...
    throw new RuntimeException("Cannot convert " + o.getClass()
        + " into a Enumerable");
  }

  /** Table that is implemented by reading from a Protobuf object. */
  private static class ProtobufTable extends AbstractQueryableTable implements
      Table {
    private final Type elementType;
    private final Enumerable enumerable;

    public ProtobufTable(Type elementType, Enumerable enumerable) {
      super(elementType);
  ...
  /**
   * Factory that creates a schema by instantiating an object and looking at its
   * public fields.
   * 
   * <p>
   * The following example instantiates a {@code FoodMart} object as a schema
  ...
  public static class Factory implements SchemaFactory {
    public Schema create(SchemaPlus parentSchema, String name,
      ...
      return new ProtobufSchema(target);
    }
  }
  ...
  private static class MethodTableMacro implements TableMacro {
    ...
    public MethodTableMacro(ProtobufSchema schema, Method method,
    ...
    public Table apply(final List<Object> arguments) {
      ...
      try {
        ...
        return new ProtobufTable(elementType, enumerable) {
          @Override
  ...
  private static class FieldTable<T> extends ProtobufTable {
  ...
  private static class AccessorTable<T> extends ProtobufTable {
    ...
    public AccessorTable(Method accessor, Type elementType, Enumerable<T> enumerable) {
      super(elementType, enumerable);
      this.accessor = accessor;
    }

    public String toString() {
      return "Relation {accessor=" + accessor.getName() + "}";
    }
