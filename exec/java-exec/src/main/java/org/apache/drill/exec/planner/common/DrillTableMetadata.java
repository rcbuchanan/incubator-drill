...
public class DrillTableMetadata {
  ...
  public Double getDistinctRowCount(List<RelDataTypeField> fields, BitSet selection) {
    ...
    if (selection.cardinality() == 0) {
      System.out.println("ignoring request for NDV of ??zero?? fields");
      return null;
    }
    
    if (selection.cardinality() > 1) {
      System.out.println("ignoring request for NDV of multiple fields");
      return null;
    }
  ...
  public void materialize(QueryContext context) throws Exception {
    ...
    ndv = new HashMap<String, Integer>();
    for (Map<String, Object> r : result) {
      ndv.put((String) r.get("column"), (Integer) r.get("ndv"));
    }
